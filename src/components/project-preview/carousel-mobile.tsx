...
export interface CarouselMobilePropsSingle {
  item: ContentItem;
  index?: number;
}

const CarouselMobileSingle: React.FC<CarouselMobilePropsSingle> = ({
  item,
  index,
}) => {
  return (
...
const CarouselMobileMany: React.FC<CarouselMobileProps> = ({ items }) => {
          ...
          <CarouselContent className="flex w-full gap-4">
            {items.map((project, index) => (
              <CarouselItem key={index} className="w-full basis-[91%]">
                <CarouselMobileSingle item={project} index={index} />
              </CarouselItem>
            ))}
          </CarouselContent>
...
const CarouselMobile: React.FC<ProjectPreviewProps> = ({ items }) => {
  return (
    <RenderConditionally condition={Array.isArray(items)}>
      <CarouselMobileMany items={items as ContentItem[]} />
      <CarouselMobileSingle item={items as ContentItem} />
    </RenderConditionally>
  );
};
